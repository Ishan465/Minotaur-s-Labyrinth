...
namespace A6MinotaurLabyrinth
{
	...
	public class MoveCommand : ICommand
	{
		...
		public void Execute(LabyrinthGame game)
		{
			...
			if (game.Map.GetRoomTypeAtLocation(newLocation) == RoomType.Pit) // if player enter pit room then kill or reduce lives according to total lives.
			{
				ConsoleHelper.WriteLine("You fall into a pit", ConsoleColor.Red);
				game.Player.ReduceLives();
                game.Map.SetRoomTypeAtLocation(newLocation, RoomType.Normal); // set that room to normal so it is not spammed

				if (game.Player.IsAlive) // if there are lives then print that player is respawned and that room is now normal
				{
                    ConsoleHelper.WriteLine("You have respawned here so this is now a normal room", ConsoleColor.Green);
                }
            }

            if (game.Map.GetRoomTypeAtLocation(newLocation) == RoomType.OneUp) // if player enter oneup room then increase life (one up name inspired from mario)
            {
				game.Player.IncreaseLife(); // increase life
				game.Map.SetRoomTypeAtLocation(newLocation, RoomType.Normal); // set that room to normal so it is not spammed
				ConsoleHelper.WriteLine("You got an extra life. Now this room is normal room", ConsoleColor.Green); // print that extra life is given
            }
        }
	}
	...
	public class GetSwordCommand : ICommand
	{
		...
		public void Execute(LabyrinthGame game)
		{
			if (game.Map.GetRoomTypeAtLocation(game.Player.Location) == RoomType.Sword) game.PlayerHasSword = true;
